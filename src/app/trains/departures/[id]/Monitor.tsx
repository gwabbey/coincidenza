...
export function Monitor({monitor}: { monitor: any }) {
    const rows = monitor.trains.map((train: any, index: number) => (<Table.Tr key={index}>
        ...
        <Table.Td><Image src={train.category.trim()} width={60} height={40} style={{
    ...
    return (<Table striped highlightOnHover stickyHeader verticalSpacing="sm" captionSide="bottom" ta="left">
