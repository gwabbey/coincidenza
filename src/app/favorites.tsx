...
export function Favorites({favorites}: { favorites: Favorite[] }) {
    ...
    function removeFavorite(favorite: Favorite) {
        ...
        const newFavorites = favorites.filter(f => f.id ? f.id !== favorite.id : (Math.abs(f.lat - favorite.lat) > 0.0001 || Math.abs(f.lon - favorite.lon) > 0.0001));
            ...
            {favorites.length ? favorites.slice()
                ...
                .map((favorite: Favorite, index) => (
                    <div key={index} className="flex justify-between items-center py-2">
                        <Link
                            href={favorite.type === "train" ? `/departures/${favorite.id ?? ""}` : "/bus"}
                            onPress={() => {
